...
Two parallel ingest entrypoints that read the **same JSONL contract** above.

### Supabase (new — GH Actions / cloud path)
```bash
python3 db/supabase_ingest.py data/crawled/*.jsonl
```
Requires `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` env. Upserts via
`db/posts.py:upsert_posts(rows)` with `on_conflict=(source, source_id)`.
No write path to Supabase should bypass this module.

### SQLite (legacy — HB's local cron)
```bash
python3 db/ingest.py data/crawled/*.jsonl
```
Keeps the existing `data/signal.db` flow intact. During the transition you can
feed the same JSONL to both entrypoints.
